...
contract RafflesContract is IERC173, IERC165 {
    ...
    event RaffleStarted(
        ...
        uint256 raffleEnd,
        RaffleItemInput[] raffleItems
    );
    event RaffleTicketsEntered(
        uint256 indexed raffleId,
        address entrant,
        TicketItemIO[] ticketItems
    );
    ...
    function requestRandomness(
        ...
        im_link.transferAndCall(
            im_vrfCoordinator,
            _fee,
            abi.encode(_keyHash, _seed)
        );
    ...
    function drawRandomNumber(uint256 _raffleId) external {
        ...
        require(
            ...
            "Raffle: Raffle time has not expired"
        );
        require(
        ...
        require(
            ...
            "Raffle: Random number is pending"
        );
    ...
    function startRaffle(
        ...
        RaffleItemInput[] calldata _raffleItems
    ) external {
        require(
            msg.sender == s.contractOwner,
            "Raffle: Must be contract owner"
        ...
        require(
            _raffleDuration >= 3600,
            "Raffle: _raffleDuration must be greater than 1 hour"
        );
        ...
        for (uint256 i; i < _raffleItems.length; i++) {
            ...
            require(
                raffleItemInput.raffleItemPrizes.length > 0,
                "Raffle: No prizes"
            );
            ...
            require(
                ...
                raffle.raffleItemIndexes[raffleItemInput.ticketAddress][
                    raffleItemInput.ticketId
                ] == 0,
            ...
            for (uint256 j; j < raffleItemInput.raffleItemPrizes.length; j++) {
                RaffleItemPrizeIO calldata raffleItemPrizeIO = raffleItemInput
                    .raffleItemPrizes[j];
                raffleItem.raffleItemPrizes.push(
                    RaffleItemPrize(
                        raffleItemPrizeIO.prizeAddress,
                        uint96(raffleItemPrizeIO.prizeQuantity),
    ...
    */
    function onERC1155Received(
        ...
        require(
            _data.length == 32,
            "Raffle: Data of the wrong size sent on transfer"
        );
        ...
        require(
            raffleEnd > block.timestamp,
            "Raffle: Can't accept transfer for expired raffle"
        );
        ...
        )
    {
        ...
        for (uint256 i; i < raffle.raffleItems.length; i++) {
            ...
            raffleItems_[i].raffleItemPrizes = new RaffleItemPrizeIO[](
                raffleItem.raffleItemPrizes.length
            );
            for (uint256 j; j < raffleItem.raffleItemPrizes.length; j++) {
                RaffleItemPrize storage raffleItemPrize = raffleItem
                    .raffleItemPrizes[j];
                raffleItems_[i]
                    .raffleItemPrizes[j]
                    .prizeAddress = raffleItemPrize.prizeAddress;
                raffleItems_[i].raffleItemPrizes[j].prizeId = raffleItemPrize
                    .prizeId;
                raffleItems_[i]
                    .raffleItemPrizes[j]
                    .prizeQuantity = raffleItemPrize.prizeQuantity;
            }
        }
    }
    ...
    function getEntries(
        uint256 _raffleId,
        address _entrant
    ) external view returns (EntryIO[] memory entries_) {
        require(_raffleId < s.raffles.length, "Raffle: Raffle does not exist");
        Raffle storage raffle = s.raffles[_raffleId];
        ...
        for (uint256 i; i < raffle.entries[_entrant].length; i++) {
            ...
            RaffleItem storage raffleItem = raffle.raffleItems[
    ...
    function ticketStats(
        uint256 _raffleId
    ) external view returns (TicketStatsIO[] memory ticketStats_) {
        require(_raffleId < s.raffles.length, "Raffle: Raffle does not exist");
        Raffle storage raffle = s.raffles[_raffleId];
    ...
    function enterTickets(
        uint256 _raffleId,
        TicketItemIO[] calldata _ticketItems
    ) external {
        require(_raffleId < s.raffles.length, "Raffle: Raffle does not exist");
        ...
        for (uint256 i; i < _ticketItems.length; i++) {
            ...
            // get the raffle item
            uint256 raffleItemIndex = raffle.raffleItemIndexes[
                ticketItem.ticketAddress
            ][ticketItem.ticketId];
            require(
                raffleItemIndex > 0,
                "Raffle: Raffle item doesn't exist for this raffle"
            );
            raffleItemIndex--;
            ...
            raffle.entries[msg.sender].push(
                Entry(
                    ...
                    false,
                    uint112(totalEntered),
                    uint112(totalEntered + ticketItem.ticketQuantity)
                )
            );
    ...
    function claimPrize(
        ...
        for (uint256 i; i < _wins.length; i++) {
            ...
            uint256 totalEntered = raffle
                .raffleItems[entry.raffleItemIndex]
                .totalEntered;
            lastValue = raffle
                .raffleItems[entry.raffleItemIndex]
                .raffleItemPrizes
            ...
            for (uint256 j; j < win.prizes.length; j++) {
                ...
                RaffleItemPrize memory raffleItemPrize = raffle
                    .raffleItems[entry.raffleItemIndex]
